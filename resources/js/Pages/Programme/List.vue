...
export default {
  ...
  mounted() {
    ...
    $(document).on("click", ".edit", (evt) => {
      ...
      router.get(route("admin.programme.edit", data));
    });
  ...
  methods: {
    fetch() {
      $("#data_table").DataTable({
        ...
        serverSide: true,
        ajax: {
          url: route("admin.programme.fetch"),
        },
        columns: [
    ...
    destroy() {
      axios
        .get(route("admin.programme.destroy", { programme: this.selectedRow }))
        .then((response) => [
          this.hideDestroyModal(),
          toastr.success("Programme successfully deleted"),
          this.fetch(),
        ])
...
<template>
  ...
  <AuthenticatedLayout>
    ...
    <template #header>
      <div class="flex items-center">Programmes</div>
    </template>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
          <div class="p-6">
            <div class="flex justify-end">
              <LinkButton :href="route('admin.programme.create')">
    ...
    <Modal
      :show="destroyModal"
      :closeable="true"
      :modalTitle="'delete programme'"
      @close="hideDestroyModal"
      :maxWidth="'md'"
    >
      <div class="flex justify-center mt-4">
        <p class="text-lg">Are you sure you want to delete this programme?</p>
      </div>
