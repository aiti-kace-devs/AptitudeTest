...
export default {
  ...
  mounted() {
    ...
    $(document).on("click", ".edit", (evt) => {
      ...
      router.get(route("admin.course_category.edit", data));
    });
  ...
  methods: {
    fetch() {
      $("#data_table").DataTable({
        ...
        serverSide: true,
        ajax: {
          url: route("admin.course_category.fetch"),
        },
        columns: [
    ...
    destroy() {
      axios
        .get(route("admin.course_category.destroy", { category: this.selectedRow }))
        .then((response) => [
...
<template>
  ...
  <AuthenticatedLayout>
    ...
    <template #header>
      <div class="flex items-center">Course Categories</div>
    </template>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
          <div class="p-6">
            <div class="flex justify-end">
              <LinkButton :href="route('admin.course_category.create')">
    ...
    >
      <div class="flex justify-center mt-4">
        <p class="text-lg">Are you sure you want to delete this category?</p>
      </div>
