                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('course_id') is-invalid @enderror" id="course_id" name="course_id" required>
                                        ...
                                        @foreach($courses as $course)
                                            <option value="{{ $course->id }}" {{ ($admission && $admission->course_id == $course->id) ? 'selected' : '' }}>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('session') is-invalid @enderror" id="session" name="session" required>
                                        ...
                                        @if($admission && $admission->session)
                                            <option value="{{ $admission->session }}" selected>{{ $admission->session }}</option>
                                        @endif
                                    </select>
                                    @error('session')
                                        <span class="invalid-feedback" role="alert">
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $.each(data.sessions, function(index, session) {
                            $('#session').append('<option value="' + session + '">' + session + '</option>');
                        });
                    },
                    error: function() {
